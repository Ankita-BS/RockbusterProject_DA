...
SELECT outer_query.country, outer_query.all_customer_count, count(top_5_customers.customer_id) AS top_customer_count FROM 
(	SELECT  D.country ,count ( DISTINCT A.customer_id) AS all_customer_count
	...
	INNER JOIN country D ON C.country_id = D.country_id
	GROUP BY  D.country
	ORDER BY count (DISTINCT A.customer_id) DESC ) AS outer_query
	LEFT JOIN
	(SELECT Cu.customer_id,Cu.first_name, Cu.last_name, D.country,C.city,
	SUM(P.amount) AS total_amount_paid
	FROM customer Cu
